    ...
    <div id="vipOperate">
        <div :class="{'disable':!selTgyVal }">
            <section :class="{'hide':!isShowNextBtn}">
                <span>请输入兑奖数目(最多可兑：{{selTgyVal.gameGold}} )：</span><el-input :disabled=!selTgyVal readonly size="small" v-model="payNum" placeholder="请输入兑奖数目"></el-input>
            ...
            <el-button :class="{'hide':!isShowNextBtn}" size="small" :disabled=!selTgyVal type="danger" v-tap="{ methods:setPay }">下一步</el-button>
        </div>
    </div>
</template>
        ...
        methods: {
            openInpPass () {
                this.$prompt('请输入密码!', '提示', {
                    ...
                    let sendPassword = await this.$store.dispatch(actionTypes.enterSelfPwd, [ value ])
                    if (sendPassword && sendPassword === true) {
                        let expiryNumber = await this.$store.dispatch(actionTypes.promoterExpiry, [ this.selTgyVal.id, Number(this.payNum), 1 ])
